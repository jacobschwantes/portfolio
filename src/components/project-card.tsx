...
export function ProjectCard2({ project }: { project: Project }) {
	return (
		<Link key={project.slug} href={`/project/${project.slug}`}>
			<div className=" bg-white dark:bg-zinc-900 rounded-2xl group flex flex-col sm:flex-row overflow-hidden w-full relative border border-zinc-100 dark:border-zinc-800 min-h-[11rem] sm:h-44 justify-between ">
				<div className="absolute top-4 right-4">
					<ArrowUpRight className="h-4 w-4 text-zinc-500 dark:text-zinc-300" />
				</div>

				<div className="w-full sm:w-auto sm:aspect-[4.5/3] h-48 sm:h-full bg-zinc-50">
				...
				<div className="flex flex-col px-5 py-4 w-full sm:pr-16">
